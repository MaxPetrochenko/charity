...
describe("Fundraising", function () {
  ...
  async function deploy() {
    const [Alice, Bob, Clayton, Dave, Donater, Withdrawer_1, Withdrawer_2] =
      await hre.ethers.getSigners();

    const Fundraising = await hre.ethers.getContractFactory("FundraisingV1");
    const fundraising = await Fundraising.deploy([Alice, Bob, Clayton], {
      from: Alice.address,
    });

    const MockToken = await hre.ethers.getContractFactory(
      "MockERC20Ownable",
      Alice
    );
  ...
  describe("Setup: ", () => {
    ...
    describe("#approveFundraising", () => {
      it("Should approve fundraising", async () => {
        const { fundraising, mock, Bob, Clayton, Withdrawer_1, Withdrawer_2 } =
          await loadFixture(deploy);
        const _Bob = fundraising.connect(Bob);
        const number_ = ethers.toNumber(5n * 10n ** 12n);
        await _Bob.registerFundrasing(
          number_,
          await mock.getAddress(),
          Withdrawer_1.address,
          [Withdrawer_1.address, Withdrawer_2.address]
        );
  ...
  describe("Settings:", () => {
    ...
    it("Should remove managers", async () => {
      ...
      await expect(contract.removeManagers([Bob.address]))
        .to.emit(fundraising, "ManagerRemoved")
  ...
  describe("#approveWithdrawal", () => {
    it("Should approve withdrawal", async () => {
      ...
      await fundraising
        .connect(Donater)
        .donate(0, await mock.getAddress(), 10000);
      await expect(fundraising.connect(Withdrawer_1).approveWithdrawal(0)).not
        .to.be.reverted;
